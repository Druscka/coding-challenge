  ...
  $.smartscroll = function smartscroll(overrides) {
    ...
    if (
      ...
      (options.eventEmitter &&
        options.eventEmitter.constructor !== EventEmitter)
    ) {
    ...
    var calculateSectionBottoms = function() {
      ...
      sectionWrapperBottom = Math.round(
        sectionWrapperTop + sectionWrapper.height(),
        10
      );
    ...
    var autoHash = function() {
      ...
      if (getWindowTop() + $(window).height() / 2 < sectionWrapperTop) {
        ...
        var slideIndex = getSectionIndexAt(
          getWindowTop() + $(window).height() / 2
        );
        if (slideIndex !== undefined) {
          // And get the data-hash attribute of the section
          newHash = $(
            options.sectionSelector + ":nth-of-type(" + slideIndex + ")"
          ).data("hash");
        }
      }
      if (
        typeof newHash === "undefined" ||
        ...
        .animate(
          ...
          function() {
            ...
            if (options.eventEmitter) {
              var windowTop = getWindowTop();
              var sectionIndexAtWindowMiddle = getSectionIndexAt(
                windowTop + $(window).height() / 2
              );
              options.eventEmitter.emitEvent("scrollEnd", [
                sectionIndexAtWindowMiddle
              ]);
            }
          }
        );
    ...
    this.scroll = function scroll(down) {
      if (sections) {
        ...
        if (options.eventEmitter) {
          sectionIndexAtWindowMiddle = getSectionIndexAt(
            windowTop + $(window).height() / 2
          );
    ...
    var bindScroll = function() {
      $(window).bind(MOUSE_EVENTS_STRING, function(e) {
        ...
        if (
          ...
          if (
            sectionIndexAtWindowTop !== sectionIndexAtWindowBottom ||
            ...
            if (scrollAction) {
              if (scrollAction === "up") {
                if (options.toptotop) {
                  ...
                  scrollToPixel(
                    sections[sectionIndexAtWindowMiddle - 1] -
                      $(window).height(),
                    options.animationSpeed
                  );
                }
                if (options.eventEmitter) {
                  options.eventEmitter.emitEvent("scrollStart", [
                    sectionIndexAtWindowMiddle - 1
                  ]);
                }
